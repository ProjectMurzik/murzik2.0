...
bool Assets::LvglStrategy::InitializePartition(Assets* assets) {
    ...
    esp_err_t err = esp_partition_mmap(assets->partition_, 0, assets->partition_->size, 
...
bool Assets::LvglStrategy::Apply(Assets* assets, bool refresh_display_theme) {
    ...
    if (cJSON_IsObject(skin)) {
        ...
        if (cJSON_IsObject(light_skin) && light_theme != nullptr) {
            ...
            if (cJSON_IsString(background_image)) {
                ...
                auto background_image_obj = std::make_shared<LvglCBinImage>(ptr);
                light_theme->set_background_image(background_image_obj);
            }
        }
        ...
        if (cJSON_IsObject(dark_skin) && dark_theme != nullptr) {
            ...
            if (cJSON_IsString(background_image)) {
                ...
                auto background_image_obj = std::make_shared<LvglCBinImage>(ptr);
                dark_theme->set_background_image(background_image_obj);
            }
        }
    }
    ...
    if (refresh_display_theme) {
        ...
        if (cJSON_IsBool(hide_subtitle)) {
            ...
            // 🔥 ЗАМЕНА dynamic_cast на GetDisplayType() для LcdDisplay
            auto lcd_display = (display && display->GetDisplayType() == Display::TYPE_LCD) 
                               ? static_cast<LcdDisplay*>(display) 
                               : nullptr;
...
bool Assets::EmoteStrategy::InitializePartition(Assets* assets) {
    ...
    // 🔥 ЗАМЕНА dynamic_cast на GetDisplayType() для EmoteDisplay
    auto* emote_display = (display && display->GetDisplayType() == Display::TYPE_EMOTE) 
                          ? static_cast<emote::EmoteDisplay*>(display) 
                          : nullptr;
...
bool Assets::Download(std::string url, std::function<void(int progress, size_t speed)> progress_callback) {
    ...
    // Отмена текущего ресурсного раздела
    UnApplyPartition();
    ...
    // Определение размера сектора (стандарт ESP32 - 4KB)
    const size_t SECTOR_SIZE = esp_partition_get_main_flash_sector_size();
    
    // Расчет количества секторов для стирания
    ...
    // Выделение буфера во внутренней памяти
    ...
    // Повторная инициализация ресурсного раздела
