...
std::string EspVideo::Explain(const std::string& question) {
    if (explain_url_.empty()) {
        ...
        // 🔥 ИСПРАВЛЕНО 1: Заменено throw на возврат строки
    ...
    if (jpeg_queue == nullptr) {
        ...
        // 🔥 ИСПРАВЛЕНО 2: Заменено throw на возврат строки
        return std::string("Error: Failed to create JPEG queue");
    }
    ...
    if (!http->Open("POST", explain_url_)) {
        ...
        // 🔥 ИСПРАВЛЕНО 3: Заменено throw на возврат строки
        return std::string("Error: Failed to connect to explain URL");
    }
    {
        // 第一块：question字段
    ...
    if (!saw_terminator || total_sent == 0) {
        ...
        // 🔥 ИСПРАВЛЕНО 4: Заменено throw на возврат строки
        return std::string("Error: Failed to encode image to JPEG");
    }
    {
        // 第四块：multipart尾部
    ...
    if (http->GetStatusCode() != 200) {
        ...
        // 🔥 ИСПРАВЛЕНО 5: Заменено throw на возврат строки
        return std::string("Error: Failed to upload photo, status code: " + std::to_string(http->GetStatusCode()));
    }
