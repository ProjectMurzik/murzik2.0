...
MipiLcdDisplay::MipiLcdDisplay(esp_lcd_panel_io_handle_t panel_io, esp_lcd_panel_handle_t panel,
    ...
    const lvgl_port_display_cfg_t disp_cfg = {
        ...
        .flags = {
            .buff_dma = true,
            .buff_spiram = true,    // ✅ Буфер в PSRAM
            .sw_rotate = true,
        },
    };
...
LcdDisplay::~LcdDisplay() {
    ...
    if (preview_image_ != nullptr) lv_obj_del(preview_image_);
    if (chat_message_label_ != nullptr) lv_obj_del(chat_message_label_);
    if (emoji_label_ != nullptr) lv_obj_del(emoji_label_);
    if (emoji_image_ != nullptr) lv_obj_del(emoji_image_);
    if (emoji_box_ != nullptr) lv_obj_del(emoji_box_);
    if (content_ != nullptr) lv_obj_del(content_);
    if (bottom_bar_ != nullptr) lv_obj_del(bottom_bar_);
    if (status_bar_ != nullptr) lv_obj_del(status_bar_);
    if (top_bar_ != nullptr) lv_obj_del(top_bar_);
...
void LcdDisplay::SetChatMessage(const char* role, const char* content) {
    ...
    if (content_ == nullptr) {
        if (setup_ui_called_) {
            ESP_LOGW(TAG, "SetChatMessage('%s', '%s') failed: content_ is nullptr", role, content);
        }
        return;
    }
...
// ============================================================================
// ✅ ИСПРАВЛЕННЫЙ SetEmotion — ключевое изменение!
// ============================================================================
void LcdDisplay::SetEmotion(const char* emotion) {
    ...
    // ✅ КРИТИЧНО: оборачиваем в lvgl_port_lock для потокобезопасности!
    //    LVGL не потокобезопасен, все операции с LVGL объектами должны быть
    //    защищены мьютексом. Вызов из потока приложения без блокировки —
    //    основная причина, почему эмоции не отображаются.
